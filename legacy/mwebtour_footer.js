...
var RERIGHT = {
  ...
  kakao_init: false,
  setParams: function setParams(obj) {
  ...
  depth3: function depth3(datas) {
    for (var i = 0; i < this.menus.length; i++) {
      var menu = this.menus[i];

      for (var j = 0; j < menu.subs.length; j++) {
        var sub = menu.subs[j];
  ...
  getAreaMenuList: function getAreaMenuList() {
    //인기플레이스 리스트
    var areaMenuData = [];
    var xhr = new XMLHttpRequest();
    xhr.open('GET', apiUrl + '/open/lctDarAre/getPopularAreasForOpen?limit=6', false);
    xhr.send();
    var PopularAreas = JSON.parse(xhr.response.replace(/^document.write\(\'/, '').replace(/\'\)\;/g, '').split('null').join('""'));
  ...
  /**
    ...
    */
  getRandomInt: function getRandomInt(start, end) {
  ...
  getShortUrl: function getShortUrl() {
    ...
    var shortUrl = '';
    var url = encodeURIComponent(RERIGHT.params.buttonUrl);

    var xhr = new XMLHttpRequest();
    xhr.open('GET', apiUrl + '/open/comLgiApiCategory/getShortUrlOpenApi?url=' + url, false);
    xhr.send();
    shortUrl = JSON.parse(xhr.response.replace(/^document.write\(\'/, '').replace(/\'\)\;/g, '').split('null').join('""'));
   ...
   * SNS 공유하기 > 문자보내기(app)
   */
  doShareSMS: function doShareSMS(msg) {
    var isIOS = isIOSApp();

    var param = '{"scheme":"common" , "tag":"sendSMS" , "params":{"phoneNumber":"","message":"' + msg + '"}}';
   ...
   * 카카오톡 공유하기 > DCR앱일 경우
   */
  doShareKakao: function doShareKakao() {
    var isIOS = isIOSApp();
    var param = '{"scheme":"common" , "tag":"kakaoLink" , "params":{"url":"' + RERIGHT.params.url + '", "label":"' + RERIGHT.params.name + '", "imageUrl":"' + RERIGHT.params.imgUrl + '", "imageWidth":"414" , "imageHeight":"414","description":"' + RERIGHT.params.description + '" }}';
  ...
  rebuild: function rebuild(datas) {
  ...
  getMenu: function getMenu() {
    ...
    if (menuData == null || menuTime == null || menuTimeExp < currTime) {
      //전체메뉴 api 호출부
      var xhr = new XMLHttpRequest();
      xhr.open('GET', apiUrl + '/open/smgSmmApiCategory/getListSmmNewApi?siteExprChnlDvCd=WTM&menuObjId=ALL', false);
      xhr.send();
      var resultData = JSON.parse(xhr.response.replace(/^document.write\(\'/, '').replace(/\'\)\;/g, '').split('null').join('""'));
  ...
  ui: {
    render: function render(menus) {
      ...
      for (var i = 0; i < menus.length; i++) {
        ...
        for (var j = 0; j < menu.subs.length; j++) {
          // 전체메뉴 > 하위 폴더가 있는 경우
          var sub = menu.subs[j]; // sub === 1depth폴더들(타이틀)
          ...
          if (sub.name == '인기플레이스') {
            //인기플레이스 영역
            var areaMenuList = RERIGHT.getAreaMenuList();

            for (var a = 0; a < areaMenuList.length; a++) {
              var areaName = areaMenuList[a].name;
              var areaDetail = areaMenuList[a].code;
              if (a === 0) {
                //<ul>시작태그 삽입
                temps[i] += '<ul>';
              }
    ...
    snsMail: function snsMail() {
      var temps = [];
      if (RERIGHT.params.prodType === 'etc') {
        temps = '';
      } else {
        temps = '<li><a href="javascript:WEBTOURFNC.sns_mail()" id="mailLink" class="sns_mail">메일공유</a></li>';
      }
      return temps;
  ...
  $(document).ready(function () {
    ...
    if (isDCR) {
      ...
      //앱이 아닐 경우, 스크롤 up/down시 하단 GNB 노출/미노출 처리
  ...
  if (location.href.indexOf('mwtdom.hanatour.com') > -1 || location.href.indexOf('mfnd.hanatour.com/ko/product/') > -1 || location.href.indexOf('dev-user-mobile.hanatour.com/ko/product/') > -1) {
  ...
  setInterval(function () {
    if (currLocation !== location.href) {
      ...
      var quickMenuOff = false;
      quickMenuOff = location.href.indexOf('mwtdom.hanatour.com') > -1 || location.href.indexOf('mfnd.hanatour.com/ko/product/') > -1 || location.href.indexOf('dev-user-mobile.hanatour.com/ko/product/') > -1;
  ...
  var getParams = '&srwrDvCd=SWD02&srwrExprDvCd=M&srwrNm='; //srwrExprDvCd : P(PC),M(모바일)

  wtjs.ajax(apiUrl + '/open/comSrhApiCategory/getListSrhTourApi?_siteId=' + webtourSiteId + getParams, { credentials: true, method: 'GET' }, function (data) {
    if (data.resultMessage === 'SUCCESS') {
      if (data.getListSrhConfig.srwrMgmtLst.length > 0) {
        var index = RERIGHT.getRandomInt(0, data.getListSrhConfig.srwrMgmtLst.length);
        document.querySelector('button.srch_find').value = data.getListSrhConfig.srwrMgmtLst[index].lnkSrwrNm; //추천키워드 세팅
        document.querySelector('button.srch_find').innerHTML = data.getListSrhConfig.srwrMgmtLst[index].srwrNm; //추천검색어 세팅
      }
    }
  });
};
...
$('#btn_aco').css({
  background: '#fff url(https://image.hanatour.com/usr/static/img2/mobile/com/ico_acc_down.png) no-repeat 100% 50%',
...
var WEBTOURFNC = {
   ...
   * 전체메뉴/통검 돋보기/전체도시/홈
  */
  rightMenuClick: function rightMenuClick() {
    //전체메뉴
    ...
    var param = '{"scheme":"dcr", "tag":"show", "params":{"label":"totalMenu","type":"animation"}}';
    if (isIOS === 'Y' && isDCR) {
      //IOS
  ...
  btnSearchClick: function btnSearchClick() {
    //통검 돋보기
    // console.log(':::: btnSearchClick() call')
    var isIOS = isIOSApp();
    var isDCR = isDCRApp();
    var param = '{"scheme":"dcr", "tag":"show", "params":{"label":"search","type":"animation"}}';
    if (isIOS === 'Y' && isDCR) {
      //IOS
  ...
  btnAllClick: function btnAllClick() {
    //전체도시
    // console.log(':::: btnAllClick() call')
    var isIOS = isIOSApp();
    var isDCR = isDCRApp();
    var param = '{"scheme":"dcr", "tag":"show", "params":{"label":"totalCity","type":"animation"}}';
    if (isIOS === 'Y' && isDCR) {
      //IOS
      ...
      var moveUrl = webtourDomain + URLS['전체도시'];
      location.href = moveUrl;
    }
  },

  //화면 이동
  go: function go(url, lnkTarget, menuName, menuId) {
    ...
    if (lnkTarget === '2') {
      ...
      if (url) {
        document.querySelector('body').classList.remove('js_right_view');
        var moveUrl = url.indexOf('http') === -1 ? webtourDomain + url : url; //일치하는 값이 없으면 -1을 반환
        location.href = moveUrl;
      } else {}
    } else {
      //화면이동 없음:"0"
  ...
  js_act_btn_search: function js_act_btn_search() {
    $('#search_word_layer.lypop_wrap.slide.fade').css('top', '0');
    $('body').css({ overflow: 'hidden', position: 'fixed', height: '100%', width: '100%' });
    $('.lypop_fade02').fadeIn(1000).addClass('fade');
    $('.lypop_fade02').fadeOut(1000).removeClass('fade');
    wtjs.clickSearch(document.querySelector('.input_keyword'), document.querySelector('div.js_tabs.link'), document.querySelector('div.list_srchword_wrap'));
  },
  //검색 버튼
  ...
  btn_aco: function btn_aco() {
    $('#footer_aco').slideToggle('slow', function (e) {
      if ($(this).css('display') === 'none') {
        ...
        $('#btn_aco').css({
          background: '#fff url(https://image.hanatour.com/usr/static/img2/mobile/com/ico_acc_up.png) no-repeat 100% 50%',
          'background-size': '1.3rem auto',
          display: 'inline-block',
          padding: '0 2rem 0 0'
        });
  ...
  search: function search(keyword) {
    var isIOS = isIOSApp();
    var isDCR = isDCRApp();
    var moveUrl = webtourDomain + '/com/its/CHPC0ITS0002M100?keyword=' + keyword + '&keywordCateg=DS';
    var param = '{"scheme":"dcr","tag":"moveURL","params":{"url":"' + moveUrl + '"}}';
    if (isIOS === 'Y' && isDCR) {
      //IOS
  ...
  //추천키워드 검색 버튼
  rcm_find: function rcm_find() {
    var isIOS = isIOSApp();
    var isDCR = isDCRApp();
    var recommendRealSearchKey = '';
    recommendRealSearchKey = document.querySelector('button.srch_find').value;
    if (recommendRealSearchKey.indexOf('http') > -1) {
      //추천키워드가 URL 형식인 경우
      var param = '{"scheme":"dcr","tag":"moveURL","params":{"url":"' + recommendRealSearchKey + '"}}';
      if (isIOS === 'Y' && isDCR) {
        //IOS
      ...
      //추천키워드가 키워드 형식인 경우
      var keyword = encodeURIComponent(recommendRealSearchKey);
      var keywordUrl = webtourDomain + '/com/its/CHPC0ITS0002M100?keyword=' + keyword + '&keywordCateg=DS';
      var _param = '{"scheme":"dcr","tag":"moveURL","params":{"url":"' + keywordUrl + '"}}';
      if (isIOS === 'Y' && isDCR) {
        //IOS
        //console.log(':::: 추천키워드(키워드형식) - IOS 호출')
        window.webkit.messageHandlers.hanatourapp.postMessage(_param);
      } else if (isDCR) {
        //AOS
        //console.log(':::: 추천키워드(키워드형식) - AOS 호출')
        window.htmlEventHandler.hanatourapp(_param);
      } else {
  ...
  enterkey: function enterkey(e) {
    if (e.keyCode == 13) {
      var text = document.querySelector('#wtjs_search').value;
      this.search(text);
    }
  },
  ...
  /* SNS 공유하기 */
  //레이어팝업 노출 처리
  js_act_btn_share: function js_act_btn_share() {
  ...
  //2. 카카오스토리
  sns_kakaostory: function sns_kakaostory() {
    //(닷컴)linkStory()
    // 모바일 앱 인 경우 웹페이지주소로 연결해야함.
    var url = RERIGHT.params.url;
    var text = '';
  ...
  //3. 페이스북
  sns_facebook: function sns_facebook() {
    //(닷컴)link()
    var isApp = isHybridApp();
    var isIOS = isIOSApp();
    var device = RERIGHT.checkDevice();
    var url = 'https://www.facebook.com/sharer.php?u=' + RERIGHT.params.url;
  ...
  //4. 메일공유
  sns_mail: function sns_mail() {
    // (닷컴) mailShare2()
    var url = RERIGHT.getShortUrl();
    var href = 'mailto:?subject=' + RERIGHT.params.name + '&body=' + RERIGHT.params.description + '%0D%0A%0D%0A' + url;
    mailLink.setAttribute('href', href);
  },

  //5. 문자보내기
  sns_mms: function sns_mms() {
   ...
   */
  /* onmouseover */
  onMouseOverLocation: function onMouseOverLocation() {
    //플레이스
    animLocation.setDirection(1);
    animLocation.play();
  },
  onMouseOverSchedule: function onMouseOverSchedule() {
    //플래너
    animSchedule.setDirection(1);
    animSchedule.play();
  },
  onMouseOverHome: function onMouseOverHome() {
    //홈
    animHome.setDirection(1);
    animHome.play();
  },
  onMouseOverMy: function onMouseOverMy() {
    //마이페이지
    animMy.setDirection(1);
    animMy.play();
  },
  onMouseOverLike: function onMouseOverLike() {
    //최근본/찜
  ...
  /* onmouseleave */
  onMouseLeaveLocation: function onMouseLeaveLocation() {
    //플레이스
    animLocation.setDirection(-1);
    animLocation.play();
  },
  onMouseLeaveSchedule: function onMouseLeaveSchedule() {
    //플래너
    animSchedule.setDirection(-1);
    animSchedule.play();
  },
  onMouseLeaveHome: function onMouseLeaveHome() {
    //홈
    animHome.setDirection(-1);
    animHome.play();
  },
  onMouseLeaveMy: function onMouseLeaveMy() {
    //마이페이지
    animMy.setDirection(-1);
    animMy.play();
  },
  onMouseLeaveLike: function onMouseLeaveLike() {
    //최근본/찜
    animLike.setDirection(-1);
    animLike.play();
  },
  ...
  //CRM 태깅(헤더)
  setCrmHeaderMenu: function setCrmHeaderMenu(menuType, menuName, menuId) {
    var isDCRMain = this.isDCRMain();
    if (!isDCRMain) {
  ...
  //CRM 태깅(하단GNB)
  setCrmMainMenu: function setCrmMainMenu(menuType, menuName, menuDepth) {
    var isDCRMain = this.isDCRMain();
    if (!isDCRMain) {
  ...
  setEvent: function setEvent(eventKey, data) {
  ...
  checkAppBuildVersion: function checkAppBuildVersion() {
    var isIOS = isIOSApp();
    try {
      var appBuildVersion = Number(navigator.userAgent.split('+')[1]);
      if (this.checkHanaApp() && isIOS === 'Y' && appBuildVersion >= 188) {
        //IOS
        return true;
      } else if (this.checkHanaApp() && isIOS === 'N' && appBuildVersion >= 817) {
        //AOS
        return true;
      }
      return false;
  ...
  checkHanaApp: function checkHanaApp() {
    return (/hanatourApp/.test(navigator.userAgent)
    );
  },
  callApp: function callApp(scheme, tag, data) {
